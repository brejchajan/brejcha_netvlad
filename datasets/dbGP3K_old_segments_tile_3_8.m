classdef dbGP3K_old_segments_tile_3_8 < dbBase
    
    methods
    
        function db = dbGP3K_old_segments_tile_3_8(whichSet)
            % whichSet is one of: train, val, test
            
            assert( ismember(whichSet, {'train', 'val', 'test'}) );
            
            db.name= sprintf('dbGP3K_old_segments_tile_3_8_%s', whichSet);
            
            paths= localPaths();
            dbRoot= paths.dsetRootGP3K_old_segments_tile_3_8;
            db.dbPath= [dbRoot, 'db/']
