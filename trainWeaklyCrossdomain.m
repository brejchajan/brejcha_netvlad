function sessionID= trainWeaklyCrossdomain(dbTrain, dbVal, varargin)
    ...
    if opts.startEpoch<2
        ...
        % --- Add my layers
        net= addLayersDag(net, opts, dbTrain, opts.layerName);
        ...
        if isempty(opts.backPropToLayer)
            ...
            if ~isnumeric( opts.backPropToLayer )
                ...
                opts.backPropToLayer= brejcha_whichLayer(net, opts.backPropToLayer);
            end
        end
        opts.backPropToLayerName= net.layers(opts.backPropToLayer).name;
        opts.backPropDepth= length(net.layers)-opts.backPropToLayer+1;
        assert( all(ismember(opts.fixLayers, brejcha_layerNamesDag(net))) );
    ...
    % --- Prepare for train
    net= brejcha_netPrepareForTrainDag(net, opts.backPropToLayer);
...
function [qFeat, dbFeat]= computeAllFeats(db, net, opts, ID, delFile)
    ...
    serialAllFeatsDag(net, db.qPath, db.qImageFns, ...
    ...
    serialAllFeatsDag(net, db.dbPath, db.dbImageFns, ...
        tmpFn, 'useGPU', opts.useGPU, 'batchSize', opts.computeBatchSize);
